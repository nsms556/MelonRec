...
def make_input4tokenizer(train_file_path, genre_file_path, tokenize_input_file_path, val_file_path=None, test_file_path=None) :
  def _wv_genre(genre):
    genre_dict = dict()
...
def sentences_to_tokens(spp, sentences) :
  tokenized_stc = []
  for sentence in sentences :
    tokens = spp.EncodeAsPieces(sentence)
...
def train_word2vec(train_file_path, val_file_path, test_file_path, genre_file_path, tokenize_input_file_path, _submit_type) :
    sentences = make_input4tokenizer(train_file_path, genre_file_path, tokenize_input_file_path, val_file_path, test_file_path)
    ...
    train_tokenizer(tokenize_input_file_path, tokenizer_name, vocab_size, method)
    
    spp = spm.SentencePieceProcessor()
    spp.Load(tokenizer_name_model)
    tokenized_sentences = sentences_to_tokens(spp, sentences)
    
    w2v_name = 'model/w2v_{}_{}_{}.model'.format(method, vocab_size, _submit_type)
    w2v_model = string2vec(tokenized_sentences, size=200, window=5, min_count=1, workers=8, sg=1, hs=1)
    w2v_model.save_model(w2v_name)
...
if __name__ == '__main__' :
    ...
    plylst_title_tag_emb = get_plylsts_embeddings(train, question, model_postfix)

    np.save('{}/plylst_w2v_emb.npy'.format(default_file_path), plylst_title_tag_emb)